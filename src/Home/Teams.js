...
function Teams() {
    ...
    const fetchTeamsData = async (teamStatus = "") => {
        try {
            const apiUrl = `https://nirmaan.iitm.ac.in:8000/api/projects${teamStatus ? `?status=${teamStatus}` : ''}`;
            ...
            <div className="mt-[88px]">
                <div className="bg-green-100">
                    <div className="pt-5 pl-5 flex">
                        <span><a href="/" className="text-gray-600 font-semibold hover:underline">Home</a></span> 
                ...
                <div className="grid md:grid-cols-4 sm:grid-cols-2 gap-10 mt-10 px-10">
                    {data.length > 0 ? (
                        data.map((item, index) => (
                            <div key={index}>
                                <div className="flex flex-col justify-center items-center p-4 font-dmsans" onClick={() => handleEstablish(item)}>
                                    <img src={url + item.profile_photo || "default-image.png"} alt={item.name} className="w-[90px] h-[70px]" /><br />
